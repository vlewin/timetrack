...
class TimetracksController < ApplicationController
  ...
  def index
    ...
    @@offset[@month.first.wday].times{|i| @month.insert(i, nil)} # empty cells if first day of month is not monday
    @month = @month.to_a.in_groups_of(7)
    ...
    respond_to do |format|
      format.html
      format.js { render :partial => "timetrack", :timetrack => @timetrack }
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @timetrack.save
        ...
        format.html { redirect_to timetracks_path({:date => @timetrack.date}), :alert => @timetrack.errors.messages[:timetrack].first }
      end
    end
  end
